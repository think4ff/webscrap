...
public class HtmlParserAuction extends HtmlParser {
	...
	// HtmlParserAuction.class.getName());
	protected static Logger logger = LoggerFactory
			.getLogger(HtmlParserAuction.class);
	 ...
	 * @return
	 */
	static public boolean doParsingAll(int tab, DataAuctionInput input,
			String html, DataAuctionOutput result) {
		if (tab == 1)
			return parseTab1All(input, html, result);
	...
	static boolean parseTab1All(DataAuctionInput input, String html,
			DataAuctionOutput result) {

		boolean rtBoolean = true;

		if (!preParsing(input, html, result)) {
			rtBoolean = false;
		} else {
	...
	private static boolean preParsing(DataAuctionInput input, String html,
			DataAuctionOutput result) {
	...
	static public boolean doParsing(int tab, DataAuctionInput input,
			String html, DataAuctionOutput result) {
		...
		if (tab == 3)
			return parseTab3(input, html, result);

		return true;
	}

	static boolean parseTab1(DataAuctionInput input, String html,
			DataAuctionOutput result) {
		int pos;
		String tempStr;
		...
		if (pos >= 0) {
			// TODO:테스트를 위한 주석
			// result.GaeSiDate = cut_value_in_nextBlock( html, "<td>", new
			// MyInt(pos), "</td>" ).replaceAll("\\.", "");
			// XXX:테스트
		...
		if (pos >= 0) {
			// tempStr = cut_value_in_nextBlock( html, "<td>20", new MyInt(pos),
			// "</td>" ).replaceAll("\\.|\\r|\\n", "").trim();
			// if( !tempStr.equals("") )
			// {
			// result.BaeDangYokuDate = "20" + tempStr.substring(0, 6); // 배당요구
			// 종기일 뒤에 다른 정보는 무시한다. "131205 (연기)"라면 앞 6자리만 자른다.
			// }
			// XXX:테스트
			result.BaeDangYokuDate = getValueOne4Table(html, 1, "배당요구종기일")
					.replaceAll("\\.", "");
		}
	...
	static boolean parseTab2(DataAuctionInput input, String html,
			DataAuctionOutput result) {
		...
		// result.IlChaBeobSaGa = cut_value_in_nextBlock(html, "\"txtright\">",
		// start_pos, "원").replaceAll("\\,", "").trim();
		// XXX:테스트
		result.IlChaBeobSaGa = getValueOne4Table(html, 0, "감정평가액").replaceAll(
				"\\,|원", "");

		ArrayList<AuctionTab2GiIlNaeYeok> GiIlList = new ArrayList<AuctionTab2GiIlNaeYeok>();

		/*
		 * // 기일 테이블 Parsing 하여 KiIl List에 담는다 while( count++<100 )// true )//
		 * 혹시 무한루프를 돌까봐.. 실제로 불가능한 값 100보다 작은동안 loop를 돌도록 하였다. 실제로는 100을 절대 만날수
		 * 없음 { AuctionTab2GiIlNaeYeok tempKiIl = new AuctionTab2GiIlNaeYeok();
		 * 
		 * tempKiIl.KiIlDate = cut_value_in_nextBlock(html,
		 * "<td class=\"txtcenter\">", start_pos, "(").replaceAll("\\.",
		 * "").trim();
		 * 
		 * if( tempKiIl.KiIlDate.equals("")) break; tempKiIl.KiIlCategory =
		 * cut_value_in_nextBlock(html, "<td>", start_pos, "</td>").trim();
		 * tempKiIl.KiIlJangSo = cut_value_in_nextBlock(html, "<td>", start_pos,
		 * "</td>").trim(); tempKiIl.MinMaeKakPrice =
		 * cut_value_in_nextBlock(html, "\"txtright\">", start_pos,
		 * "</td>").replaceAll("\\,|원", "").trim(); tempKiIl.KiIlResult =
		 * cut_value_in_nextBlock(html, "<td>", start_pos,
		 * "</td>").replaceAll("\\t", "").trim();
		 * 
		 * // 기일결과에 금액이 있는 경우 if( tempKiIl.KiIlResult.indexOf("원") >= 0 ) {
		 * tempKiIl.KiIlResult_price =
		 * cut_value_in_nextBlock(tempKiIl.KiIlResult, "(", new MyInt(0),
		 * "원").replaceAll("\\,", "").trim(); } // 기일결과에 날짜가 있는 경우 else if(
		 * tempKiIl.KiIlResult.indexOf("(") >= 0 ) { tempKiIl.KiIlResult_date =
		 * cut_value_in_nextBlock(tempKiIl.KiIlResult, "(", new MyInt(0),
		 * ")").replaceAll("\\.", "").trim(); }
		 * 
		 * // 기일결과에 날짜나 금액이 있는 경우 공백까지 자른다. 예) 매각 (1,000원) 경우 '매각'만 자른다. if(
		 * tempKiIl.KiIlResult.indexOf("(") >= 0 ) { tempKiIl.KiIlResult =
		 * cut_value_in_nextBlock(tempKiIl.KiIlResult, "", new MyInt(0),
		 * " ").replaceAll("\\.", "").trim(); }
		 * 
		 * logger.info( tempKiIl.toString() ); KiIL_list.add(tempKiIl); }
		 */
		...
		for (int i = 0; i < GiIlList.size(); i++) {
			...
			if (GiIlList.get(i).KiIlCategory.equals("매각기일")) {
				...
				// 공백으로 처리
				else if (GiIlList.get(i).KiIlResult.equals("진행")
						|| GiIlList.get(i).KiIlResult.equals("")) {
					result.YipChalResultList.add("");
				} else // 그외 결과일때는 예외상황이며 공백으로 처리함.
				{
					result.YipChalResultList.add("");
					logger.error("다음 입찰결과를 공백으로 처리합니다. --> "
							+ GiIlList.get(i).KiIlResult);
				}
			}
	...
	static boolean parseTab3(DataAuctionInput input, String html,
			DataAuctionOutput result) {
		// 송달내역 -> 메모
		int last_pos;
		String tmpDate;
		String tmpJeobSuNaeYeok;
		String[] keywords = new String[] { "한국씨티은행", "배당요구", "유치권", "임차인",
				"임금채권", "근로복지공단" };
		MyInt start_pos = new MyInt();
		...
		start_pos.setValue(html.indexOf("\"txtcenter\">결과</th>")
				+ "\"txtcenter\">결과</th>".length());

		// 다음 날짜 찾기
		tmpDate = cut_value_in_nextBlock(html, "\"txtcenter\">", start_pos,
				"</td>").replaceAll("\\.", "").trim();
		// 다음 내용 찾기
		tmpJeobSuNaeYeok = cut_value_in_nextBlock(html, "<td>", start_pos,
				"</td>").replaceAll("\\t", "").trim();
		...
		while (last_pos > start_pos.getValue()) {
			for (int i = 0; i < keywords.length; i++) {
				if (tmpJeobSuNaeYeok.indexOf(keywords[i]) >= 0) {
					result.MemoList.add(tmpDate + " "
							+ tmpJeobSuNaeYeok.replaceAll(",", ""));
					break;
				}
			}
			// 다음 날짜 찾기
